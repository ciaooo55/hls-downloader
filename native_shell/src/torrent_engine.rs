...
pub fn parse_torrent_file(bytes: &[u8]) -> Result<TorrentMeta, String> {
    ...
    if length == 0 && !pieces.is_empty() {
        return Err("torrent piece hash list exists without payload length".into());
    }
    if length > 0 {
        if piece_length == 0 {
...
fn download_torrent_with_telemetry(
    ...
    if !http_seeds.is_empty() {
        reporter(TorrentTelemetry::default());
        let mut last = "torrent web seed download failed".to_string();
        for seed in http_seeds {
            ...
            last = match run_job(&job) {
                Ok(()) => match verify_web_seed_pieces(output, &meta) {
                    Ok(()) => {
                ...
                Err(
                    error @ (crate::http_engine::EngineError::Pause
                    | crate::http_engine::EngineError::Cancel),
    ...
    for _ in 0..24 {
        ...
        if body[0] == 0 {
            ...
            if let Some(size) = dict.get(b"metadata_size".as_ref()).and_then(BValue::as_int) {
                if size > MAX_UT_METADATA_BYTES as i64 {
                    return Err(format!(
                        "peer metadata_size {size} 超过上限 {MAX_UT_METADATA_BYTES}"
                    ));
                }
                metadata_size = usize::try_from(size.max(0))
                    .map_err(|_| format!("peer metadata_size {size} 无法表示"))?;
            }
...
fn verify_web_seed_pieces(path: &Path, meta: &TorrentMeta) -> Result<(), String> {
    ...
    let actual_length = fs::metadata(path)
        .map_err(|error| format!("read web seed payload length: {error}"))?
        .len();
    if actual_length != meta.length {
        return Err(format!(
            "web seed payload length mismatch: expected {}, got {actual_length}",
            meta.length
        ));
    }
...
mod tests {
    ...
    #[test]
    fn torrent_rejects_unsafe_piece_metadata() {
        let hash = [0u8; 20];
        let mut oversized = format!(
            "d4:infod6:lengthi1e4:name4:demo12:piece lengthi{}e6:pieces20:",
            MAX_PIECE_LENGTH + 1
        )
        .into_bytes();
        oversized.extend_from_slice(&hash);
        oversized.extend_from_slice(b"ee");
        assert!(parse_torrent_file(&oversized)
            .unwrap_err()
            .contains("piece length"));

        let mut lengthless =
            b"d4:infod6:lengthi0e4:name4:demo12:piece lengthi1e6:pieces20:".to_vec();
        lengthless.extend_from_slice(&hash);
        lengthless.extend_from_slice(b"ee");
        assert!(parse_torrent_file(&lengthless)
            .unwrap_err()
            .contains("without payload length"));
    }

    #[test]
    fn web_seed_verification_rejects_corruption_and_extra_bytes() {
        let dir = std::env::temp_dir().join(format!("hls-web-seed-verify-{}", std::process::id()));
        ...
        let meta = TorrentMeta {
            ...
            pieces: vec![crate::crypto_lite::sha1(payload)],
            length: payload.len() as u64,
            files: Vec::new(),
        };

        fs::write(&path, payload).unwrap();
        verify_web_seed_pieces(&path, &meta).unwrap();
        fs::write(&path, b"corrupt!").unwrap();
        assert!(verify_web_seed_pieces(&path, &meta)
            .unwrap_err()
            .contains("SHA1"));
