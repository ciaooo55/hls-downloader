...
//!
//! Windows HWND / tray UI is not implemented here. This crate exists so a
//! future supervisor binary speaks the same frames the Python core already
//! tests.
...
pub fn paint_snapshot(handoff: &Value) -> Value {
    ...
    let size = source
        ...
        .or_else(|| source.get("size").and_then(Value::as_str).and_then(|s| s.parse().ok()))
        .unwrap_or(0)
        .max(0);
...
pub fn decode_frame(buffer: &[u8]) -> Result<Value, String> {
    ...
    let message: Value = serde_json::from_slice(&buffer[4..4 + length]).map_err(|err| err.to_string())?;
