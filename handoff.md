...
- The reviewed PR hardening tail (proxy identity, Windows POST body parity, duplicate request identity, stalled FTP/FTPS cancellation, Native Host credential rollback, Metalink XML entities) is now closed on this branch. Continue source work only for a newly demonstrated correctness failure; do not invent release evidence or speculative refactors.
