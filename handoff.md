...
- Temporary repair workflows/scripts for both fixes are removed after their tested source commits land; do not treat those transient workflow commits as product surface.
- Next code-level hardening target from the reviewed PR follow-ups is duplicate-reuse request identity: same-URL tasks must not reuse stale method/credential/replay/header/body state. Keep any fix focused in `download_worker.rs` and add exact identity regressions before changing reuse behavior.
