...
- Temporary repair workflows/scripts used for focused Windows validation are removed in the same tested commits; they are not product surface.
...
- Candidate #387 on the HLS-validated connector head was intentionally superseded by the subsequent migration/FTP/recognition source-hardening commits; its cancellation is not a product failure. This connector-authored handoff commit establishes the final validation head so v7 CI, Maintenance Security and Candidate Package can run once against all accumulated source changes.
