...
`native_shell/Cargo.lock`、`native_ui/Cargo.lock`、`extension/pnpm-lock.yaml` 记录 v6 产品依赖。`requirements-release.lock`、`frontend/pnpm-lock.yaml` 与 `frontend/src-tauri/Cargo.lock` 仍服务冻结的 5.x 规格测试。发布或再分发前应根据实际打进安装包的组件重新生成 SBOM，并保留第三方许可证文件。
