...
export const pushTaskToTvbox = (taskId: string, endpoint = '') => request<{ ok: boolean; endpoint: string; share: { id: string; url: string; filename: string; size: number; expires_in_seconds: number; idle_cleanup_seconds: number } }>('/tvbox/push-task', { method: 'POST', body: JSON.stringify({ task_id: taskId, endpoint }) })
