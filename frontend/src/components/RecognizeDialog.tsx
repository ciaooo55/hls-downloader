...
import { createTask, recognizeUrl } from '../api'
...
export default function RecognizeDialog({ settings, initialUrl = '', onClose, onAdded, onNeedUserscript }: { settings: Settings; initialUrl?: string; onClose: () => void; onAdded: () => void; onNeedUserscript: () => void }) {
  ...
  const startCandidate = async (candidate: string) => {
    await createTask({ url: candidate, filename, concurrency, referer, origin, user_agent: userAgent, cookie })
    onAdded(); onClose()
  }
  ...
  return <div className="modal-overlay" onMouseDown={onClose}><section className="modal recognize-modal" onMouseDown={event => event.stopPropagation()}>
    <header><div><h2>新建下载</h2><p>支持 m3u8 直链和普通视频网页</p></div><button className="icon-button" title="关闭" onClick={onClose}><X size={18} /></button></header>
    <label>链接</label><div className="url-entry"><Link size={18} /><input autoFocus value={url} onChange={event => setUrl(event.target.value)} onKeyDown={event => { if (event.key === 'Enter') recognize() }} placeholder="粘贴 m3u8 或普通网页地址" /></div>
    ...
    <button className="text-button" onClick={() => setAdvanced(value => !value)}>{advanced ? '收起请求选项' : '请求选项（Referer / Cookie）'}</button>
    {advanced && <div className="advanced-grid"><input value={referer} onChange={event => setReferer(event.target.value)} placeholder="Referer" /><input value={origin} onChange={event => setOrigin(event.target.value)} placeholder="Origin" /><input value={userAgent} onChange={event => setUserAgent(event.target.value)} placeholder="User-Agent" /><input value={cookie} onChange={event => setCookie(event.target.value)} placeholder="Cookie" /></div>}
