...
export default function BatchAddPanel({ settings, onAdded }: { settings: any; onAdded: () => void }) {
  ...
  return <div className="batch-form">
    ...
    <div className="batch-options"><input placeholder="Referer（可选）" value={referer} onChange={event => setReferer(event.target.value)} /><label>并发</label><input className="number-input" type="number" min={1} max={64} value={concurrency} onChange={event => setConcurrency(Math.max(1, Math.min(64, Number(event.target.value))))} /><button className="primary-button" onClick={submit} disabled={loading || !urls.length}>{loading ? '添加中...' : `添加 ${urls.length} 项`}</button></div>
