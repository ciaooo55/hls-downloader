      ...
      <div className="settings-body">
        {activeSection === 'general' && <div id="settings-general" role="tabpanel" aria-labelledby="settings-tab-general" className="settings-page">
          ...
          <section className="settings-group settings-grid-group">
            ...
            <label className="checkbox-label settings-checkbox"><input type="checkbox" checked={settings.download_progress_window_enabled !== false} onChange={event => update('download_progress_window_enabled', event.target.checked)} />下载时显示进度小窗</label>
            <label className="checkbox-label settings-checkbox"><input type="checkbox" checked={settings.download_complete_popup_enabled !== false} onChange={event => update('download_complete_popup_enabled', event.target.checked)} />下载完成后弹出置顶提示</label>
            <div className="settings-field settings-field-wide"><p>进度小窗停在屏幕右下角；完成后的提示会置顶，可打开文件或所在目录。点关闭只收起窗口，下载继续。</p></div>
