...
export default function UserscriptDialog({ onClose }: { onClose: () => void }) {
  ...
  const install = async () => { const result = await openUserscriptInstaller(); setMessage(result.ok ? '已打开浏览器，请在 Tampermonkey 中确认安装。' : result.error || '无法打开安装地址') }
  ...
  return <div className="modal-overlay" onMouseDown={onClose}><section className="modal userscript-modal" onMouseDown={event => event.stopPropagation()}>
    <header><div><h2>油猴脚本工具</h2><p>用于嗅探浏览器中动态加载的 m3u8</p></div><button className="icon-button" title="关闭" onClick={onClose}><X size={18} /></button></header>
    ...
    <p className="fine-print">程序无法直接读取 Tampermonkey 的安装列表；状态表示脚本最近是否向本地下载器报到。</p>
  </section></div>
}
