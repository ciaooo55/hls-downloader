...
export const REQUEST_FIELD_HELP = {
  ...
  concurrency: '单个任务同时下载的分片数量。默认 12，最高 64；实际连接数还会受全局和单站连接预算限制。过高可能触发网站限速或占用更多资源。普通文件服务器不支持 Range 时自动使用单连接。',
  maxTasks: '同时处于下载状态的任务数量，其余任务排队等待（默认 3）。排队任务可在列表右键调整优先级。',
  speedLimit: '全局下载限速（KiB/s）。0 表示不限速；HTTP/HLS 分片共享该预算。可选的分时段限速默认关闭，开启后只在设定时段覆盖该值。',
} as const
