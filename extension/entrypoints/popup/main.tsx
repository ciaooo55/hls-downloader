...
function App() {
  ...
  useEffect(() => {
    pendingRef.current = pendingHandoffs
  }, [pendingHandoffs])
    ...
    const timer = window.setInterval(() => {
      ...
      void Promise.all(entries.map(async ([resourceId, handoffId]) => {
        try {
          ...
          setSending(current => ({ ...current, [resourceId]: handoffStatusLabel(status) }))
          ...
          // Keep waiting; popup may reopen after the native host briefly restarts.
        }
      }))
    }, 1000)
  ...
  const send = async (item: MediaResource) => {
    ...
    try {
      ...
      setSending(current => ({ ...current, [item.id]: '待确认' }))
      setPendingHandoffs(current => ({ ...current, [item.id]: response.handoff.id }))
    } catch (reason) {
  ...
  return <main>
    <header><div className="brand"><img src="/icon-32.png" alt=""/><div><h1>HLS Downloader</h1><span className={online ? 'online' : ''}>{online ? '桌面端已连接' : '桌面端离线'}</span></div></div><div className="header-actions"><button title="打开桌面端" onClick={() => browser.runtime.sendMessage({ type: 'activate' })}><ExternalLink size={14}/></button><button className="close-button" title="关闭" onClick={() => window.close()}><X size={15}/></button></div></header>
    <div className="controls"><button onClick={async () => { const value = !enabled; setEnabled(value); await browser.storage.local.set({ enabled: value }) }}><Power size={14}/>{enabled ? '自动接管已开启' : '自动接管已关闭'}</button><button onClick={toggleCookie}><ShieldCheck size={14}/>{authorized.includes(host) ? '已授权本站 Cookie' : '授权本站 Cookie'}</button></div>
...
function ResourceRow({ item, status, onSend }: { item: MediaResource; status?: string; onSend: () => void }) {
  ...
  const locked = status === '发送中' || status === '待确认' || status === '确认中' || status === '已加入'
  return <article><div><strong title={item.filename || item.title || item.url}>{item.title || item.filename || item.url.split('/').pop() || item.url}</strong><span>{[item.kind.toUpperCase(), quality, resolution, bandwidth, size, item.statusCode ? `HTTP ${item.statusCode}` : '', item.method].filter(Boolean).join(' · ')}</span><small title={[item.mimeType, host].filter(Boolean).join(' · ')}>{[item.mimeType, host].filter(Boolean).join(' · ')}</small><small className="resource-url" title={item.url}>{item.url}</small></div><button disabled={locked} title="发送到下载器" onClick={onSend}>{status ? <em>{status}</em> : <Download size={14}/>}</button></article>
}
