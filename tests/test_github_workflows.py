...
def test_release_builds_only_windows_assets_and_publishes_tags():
    ...
    assert 'default: "3.0.20"' in workflow
...
def test_readme_documents_windows_release_assets():
    ...
    assert "HLSDownloader-v3.0.20-Windows-x64-Setup.exe" in readme
    assert "HLSDownloader-v3.0.20-Windows-x64-Portable.zip" in readme
    assert "m3u8-sniffer.user.js" not in readme
    assert "https://addons.mozilla.org/zh-CN/firefox/addon/hls_downloader/" in readme
    assert "HLSDownloader-v3.0.20-Firefox-Unsigned.zip" not in readme
    assert "HLSDownloader-v3.0.20-Firefox-Source.zip" not in readme
