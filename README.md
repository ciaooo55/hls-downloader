...
[下载最新版](https://github.com/ciaooo55/hls-downloader/releases/latest) · [Firefox 插件](https://addons.mozilla.org/zh-CN/firefox/addon/hls_downloader/) · [快速开始](#-快速开始) · [浏览器插件](#-浏览器插件) · [源码开发](#-源码开发) · [发布说明](docs/releases/v3.0.23.md)
...
| 获取方式 | 适用场景 |
| --- | --- |
| `HLSDownloader-v3.0.23-Windows-x64-Setup.exe` | 推荐；支持开始菜单、卸载入口和应用内更新 |
| `HLSDownloader-v3.0.23-Windows-x64-Portable.zip` | 免安装；完整解压后运行，数据保存在便携目录 |
| `HLSDownloader-v3.0.23-Chrome-Edge-Extension.zip` | Chrome、Edge、Brave、Chromium、Vivaldi、Opera 的 MV3 扩展 |
...
- 已完成文件、HTTP/Torrent 连续范围和 HLS/DASH 已下载分片可直接投屏或 TVBox 推送；设备访问的是受限 LAN 地址，不会重新下载原始 CDN 链接。
...
1. 下载并解压 `HLSDownloader-v3.0.23-Chrome-Edge-Extension.zip`。
...
.\scripts\build_installer.ps1 -Version 3.0.23
```

输出位于被 Git 忽略的 `release/`：

```text
HLSDownloader-v3.0.23-Windows-x64-Setup.exe
HLSDownloader-v3.0.23-Windows-x64-Portable.zip
```
