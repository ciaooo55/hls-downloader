...
| 文件 | 用途 |
| --- | --- |
| `HLSDownloader-v3.0.11-Windows-x64-Setup.exe` | Windows 10/11 x64 安装版，带在线更新、卸载程序和快捷方式 |
| `HLSDownloader-v3.0.11-Windows-x64-Portable.zip` | Windows 10/11 x64 便携版，解压后直接运行 |
...
.\scripts\build_installer.ps1 -Version 3.0.11
```

输出位于忽略的 `release` 目录：

```text
HLSDownloader-v3.0.11-Windows-x64-Setup.exe
HLSDownloader-v3.0.11-Windows-x64-Portable.zip
```
