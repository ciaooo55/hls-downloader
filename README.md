...
[下载最新版](https://github.com/ciaooo55/hls-downloader/releases/latest) · [Firefox 插件](https://addons.mozilla.org/zh-CN/firefox/addon/hls_downloader/) · [快速开始](#-快速开始) · [浏览器插件](#-浏览器插件) · [源码开发](#-源码开发) · [发布说明](docs/releases/v5.0.6.md)
...
| 获取方式 | 适用场景 |
| --- | --- |
| `HLSDownloader-v5.0.6-Windows-x64-Setup.exe` | 推荐；支持开始菜单、卸载入口和应用内更新 |
| `HLSDownloader-v5.0.6-Windows-x64-Portable.zip` | 免安装；完整解压后运行，数据保存在便携目录 |
| `HLSDownloader-v5.0.6-Chrome-Edge-Extension.zip` | Chrome、Edge、Brave、Chromium、Vivaldi、Opera 的 MV3 扩展 |
...
1. 下载并解压 `HLSDownloader-v5.0.6-Chrome-Edge-Extension.zip`。
...
.\scripts\build_installer.ps1 -Version 5.0.6
```

输出位于被 Git 忽略的 `release/`：

```text
HLSDownloader-v5.0.6-Windows-x64-Setup.exe
HLSDownloader-v5.0.6-Windows-x64-Portable.zip
```
