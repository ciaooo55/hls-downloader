...
本项目使用 GitHub Actions 从源码生成 Compose Desktop Windows 安装版、便携版、Chrome/Firefox 浏览器插件和 SHA256 校验文件。工作流使用 JDK 21 构建原生界面、PyInstaller 构建无窗口下载核心；`bin/` 与 `release/` 由 `.gitignore` 排除，不应手动提交二进制产物。
...
- 测试失败：先在本机运行 `python -m pytest -q`、`desktop-compose\gradlew.bat test`、`pnpm test` 和 `pnpm run build`。
